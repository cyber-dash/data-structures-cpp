...
struct AdjacencyListEdge {
  ...
  AdjacencyListEdge(int ending_vertex_index, TWeight weight):
    ending_vertex_index(ending_vertex_index), weight(weight), next(NULL) {}

  /*! @brief 重载!= */
  bool operator != (AdjacencyListEdge<TVertex, TWeight>& edge) const {
  ...
  TWeight weight; //!< 边权重
  AdjacencyListEdge<TVertex, TWeight>* next; //!< 下一邻接边
};
...
struct AdjacencyList {
  ...
  AdjacencyListEdge<Vertex, Weight>* adjacency_list; //!< 邻接表
};
...
public:
  ...
  // 插入边
  bool InsertEdge(TVertex vertex1, TVertex vertex2, TWeight weight);
 ...
 */
template<class Vertex, class Weight>
AdjacencyListGraph<Vertex, Weight>::AdjacencyListGraph(int max_vertex_count, Weight max_weight): max_weight_(max_weight) {

  this->max_vertex_count_ = max_vertex_count;
  this->vertex_count_ = 0;
  this->edge_count_ = 0;

  this->adjacency_lists_ = new AdjacencyList<Vertex, Weight>[this->max_vertex_count_];

  for (int i = 0; i < this->max_vertex_count_; i++) {
    this->adjacency_lists_[i].adjacency_list = NULL;
  }
}
...
AdjacencyListGraph<Vertex, Weight>::~AdjacencyListGraph() {
  ...
  for (int i = 0; i < this->vertex_count_; i++) {
    ...
    AdjacencyListEdge<Vertex, Weight>* cur_edge = this->adjacency_lists_[i].adjacency_list;

    while (cur_edge != NULL) {
      this->adjacency_lists_[i].adjacency_list = cur_edge->next;
      delete cur_edge;
      cur_edge = this->adjacency_lists_[i].adjacency_list;
    }
  }
...
bool AdjacencyListGraph<Vertex, Weight>::GetWeightByVertexIndex(Weight& weight, int v1_index, int v2_index) {
    ...
    AdjacencyListEdge<Vertex, Weight>* edge = this->adjacency_lists_[v1_index].adjacency_list;
...
bool AdjacencyListGraph<Vertex, Weight>::RemoveVertex(Vertex vertex) {
  ...
  while (this->adjacency_lists_[vertex_index].adjacency_list != NULL) {

    AdjacencyListEdge<Vertex, Weight>* prior = NULL;
    AdjacencyListEdge<Vertex, Weight>* cur = this->adjacency_lists_[vertex_index].adjacency_list;
    int cur_vertex_index = cur->ending_vertex_index;

    AdjacencyListEdge<Vertex, Weight>* delete_edge = this->adjacency_lists_[cur_vertex_index].adjacency_list;
    ...
    if (delete_edge != NULL) {
      if (prior == NULL) {
        this->adjacency_lists_[cur_vertex_index].adjacency_list = delete_edge->next;
      } else {
    ...
    this->adjacency_lists_[vertex_index].adjacency_list = cur->next;
  ...
  this->adjacency_lists_[vertex_index].adjacency_list = this->adjacency_lists_[this->vertex_count_].adjacency_list;

  AdjacencyListEdge<Vertex, Weight>* cur = this->adjacency_lists_[this->vertex_count_].adjacency_list;
  while (cur != NULL) {

    AdjacencyListEdge<Vertex, Weight>* modify_edge = this->adjacency_lists_[cur->ending_vertex_index].adjacency_list;
  ...
  this->adjacency_lists_[this->vertex_count_].adjacency_list = NULL;
 ...
 */
template<class TVertex, class TWeight>
bool AdjacencyListGraph<TVertex, TWeight>::InsertEdge(TVertex vertex1, TVertex vertex2, TWeight weight) {

  int vertex1_index = this->GetVertexIndex(vertex1);
  int vertex2_index = this->GetVertexIndex(vertex2);

  // 检查vertex1和vertex2合法性, 是否在图中
  if (vertex1_index < 0 || vertex1_index >= this->vertex_count_ || vertex2_index < 0 || vertex2_index >= this->vertex_count_) {
    return false;
  }

  // 在邻接表中找 vertex1_index --> vertex2_index 是否存在, 如果存在说明边已经存在, 不能插入
  AdjacencyListEdge<TVertex, TWeight>* cur_edge = this->adjacency_lists_[vertex1_index].adjacency_list;
  ...
  AdjacencyListEdge<TVertex, TWeight>* v2_dest_edge = new AdjacencyListEdge<TVertex, TWeight>(); // vertex1 --> vertex2
  AdjacencyListEdge<TVertex, TWeight>* v1_dest_edge = new AdjacencyListEdge<TVertex, TWeight>(); // vertex2 --> vertex1
  /* error handler */

  // 更新邻接表内对应的数据
  v2_dest_edge->ending_vertex_index = vertex2_index;
  v2_dest_edge->ending_vertex = vertex2;
  v2_dest_edge->weight = weight;
  v2_dest_edge->next = this->adjacency_lists_[vertex1_index].adjacency_list;
  this->adjacency_lists_[vertex1_index].adjacency_list = v2_dest_edge;

  v1_dest_edge->ending_vertex_index = vertex1_index;
  v1_dest_edge->ending_vertex = vertex1;
  v1_dest_edge->weight = weight;
  v1_dest_edge->next = this->adjacency_lists_[vertex2_index].adjacency_list;
  this->adjacency_lists_[vertex2_index].adjacency_list = v1_dest_edge;
...
bool AdjacencyListGraph<V, W>::RemoveEdge(V vertex1, V vertex2) {
  ...
  AdjacencyListEdge<V, W>* delete_edge; // 待删除结点
  AdjacencyListEdge<V, W>* prior_edge; // 待删除结点的前一节点
  AdjacencyListEdge<V, W>* first_edge; // 第一个邻接结点就是待删除结点

  // vertex1 --> vertex2做删除
  delete_edge = this->adjacency_lists_[vertex1_index].adjacency_list;
  prior_edge = NULL;
  first_edge = this->adjacency_lists_[vertex1_index].adjacency_list;
  ...
  if (delete_edge != NULL) {
    if (first_edge == delete_edge) { // 如果第一个邻接结点就是待删除结点
      this->adjacency_lists_[vertex1_index].adjacency_list = delete_edge->next;
    } else { // 第一个结点不是待删除结点
  ...
  // vertex2 --> vertex1做删除
  delete_edge = this->adjacency_lists_[vertex2_index].adjacency_list;
  prior_edge = NULL;
  first_edge = this->adjacency_lists_[vertex2_index].adjacency_list;
  ...
  if (delete_edge != NULL) {
    if (first_edge == delete_edge) {
      this->adjacency_lists_[vertex2_index].adjacency_list = delete_edge->next;
    } else {
      prior_edge->next = delete_edge->next;
...
bool AdjacencyListGraph<V, W>::GetFirstNeighborVertex(V& first_neighbor, const V& vertex) {
  ...
  if (vertex_index >= 0) {
    AdjacencyListEdge<V, W>* edge = this->adjacency_lists_[vertex_index].adjacency_list;
    if (edge != NULL) {
...
bool AdjacencyListGraph<V, W>::GetNextNeighborVertex(V& next_neighbor, const V& vertex, const V& neighbor_vertex) {
  ...
  if (vertex_index >= 0) {
    ...
    AdjacencyListEdge<V, W>* edge_ptr = this->adjacency_lists_[vertex_index].adjacency_list;
