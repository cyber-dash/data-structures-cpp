//
// Created by fenixlee@163.com(douyin: cyberdash_yuan) on 2020/7/14.
//
  ...
  if (isGenListBegin) {
    ...
    if (isGenListName) { // 大写字母
      ...
      if (in_vector_ptr != NULL) {
        ...
        list_node_ptr->union_info_.ref_node_ptr_ = in_vector_ptr; // 指到一个已经存在的表
        in_vector_ptr->union_info_.ref_count_++; // 引用数+1

        this->LeftBracketRegulator_(in, chr);

        inExistedList = true;
        CreateList(in, in_vector_ptr->next_, inExistedList);
        ...
        list_node_ptr->union_info_.ref_node_ptr_ = new GenListNode<T>(); // 创建一个子表的0节点
        list_node_ptr->union_info_.ref_node_ptr_->union_info_.ref_count_ = 1; // 引用数+1

        CreateList(in, list_node_ptr->union_info_.ref_node_ptr_->next_, inExistedList);
        CreateList(in, list_node_ptr, inExistedList);
      }
    }
    ...
    CreateList(in, list_node_ptr->next_, inExistedList);
  } else if (chr == ')') {
    if (!inExistedList) {
      list_node_ptr->next_ = NULL;
    }
    inExistedList = false;
  } else if (chr == '#') {
    if (!inExistedList) {
...
istream& operator>>(istream& in, GenList<T>& gen_list) {
  ...
  gen_list.CreateList(in, gen_list.ref_node_ptr_->next_, inExistedList);
