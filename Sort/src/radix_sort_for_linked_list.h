...
struct radix_static_linked_list_t {
    StaticLinkedListNode<DIGIT_TYPE> elements[100];    //!< 元素的静态链表, elements[0]为头结点(不保存元素)
...
template <typename DIGIT_TYPE>
void DistributeIntoBuckets(radix_static_linked_list_t<DIGIT_TYPE>* static_linked_list,
                           int place_of_digit,
                           ...
                           int* digit_bucket_tails)
{
    ...
    /// ### 3 遍历静态链表, 执行分配 ###
    StaticLinkedListNode<DIGIT_TYPE>* elements = static_linked_list->elements;
 ...
 */
template <typename DIGIT_TYPE>
void CollectBuckets(StaticLinkedListNode<DIGIT_TYPE>* elements,
                    int* digit_bucket_heads,
                    int* digit_bucket_tails)
{
 ...
 */
template <typename DIGIT_TYPE>
void RadixSort(radix_static_linked_list_t<DIGIT_TYPE>* static_linked_list) {
...
template <typename DIGIT_TYPE>
void RadixArrayOutput(StaticLinkedListNode<DIGIT_TYPE>* static_linked_list, int static_linked_list_length) {
    /// 遍历静态链表, 打印keys
    int index = 0;
